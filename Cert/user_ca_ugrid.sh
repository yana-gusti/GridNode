...
CERTTYPE='people'
ENTITY='Iana Kokriashkina'
ORGUNIT='UzhNU'
FIRSTNAME='Iana'
LASTNAME='Kokriashkina'
...
[ req_dn ]
EOF
openssl x509 -noout -subject -nameopt multiline -nameopt sname -in "$OLDCERT" |
( di=0 ; oi=0 ;
while read x ; do case "$x" in
DC*) echo "$di.$x" ; di=`expr $di + 1` ;;
OU* ) echo "0.$x" ;;
O* ) echo "$oi.$x" ; oi=`expr $oi + 1` ;;
CN* ) echo "0.$x" ;;
esac
done
echo "0.emailAddress = $EMAIL"
) >> "$CONF"
...
echo "$MAIL"
echo "using the web interface at https://ca.ugrid.org/"
echo ""
